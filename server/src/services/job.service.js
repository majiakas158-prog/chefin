...
const jobSelect = {
  ...
  postedBy: {
    select: {
      id: true,
      name: true,
      profile: { select: { restaurantName: true, city: true } },
    },
  },
...
export const applyToJob = (chefId, jobId) =>
...
export async function dashboard(userId) {
  ...
  const [applications, jobs] = await Promise.all([
    prisma.application.count({ where: { chefId: userId } }),
    listJobs(),
  ]);
  return { role: "chef", profile, jobs, stats: { appliedJobs: applications } };
}
